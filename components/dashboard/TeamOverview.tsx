...
interface TeamOverviewProps {
  members: (ProjectMember & { user: { id: string; display_name: string | null; email: string | null; avatar_url: string | null } })[];
...
export function TeamOverview({
  ...
  const activeMembers = members.filter(m => m.user_id !== currentUserId).length;
  const isCurrentUserInTeam = members.some(m => m.user_id === currentUserId);
            ...
            .map((member) => (
              <div
                ...
                className={cn(
                  ...
                  // Highlight if this is the current user (though we filtered them out above)
                  member.user_id === currentUserId ? "border-border-success/30 bg-border-success/10" : ""
                )}
              >
                <div className="flex-shrink-0">
                  <Image
                    src={member.user.avatar_url ?? "/default-avatar.png"}
                    alt={member.user.display_name ?? member.user.email ?? 'User'}
                    width={32}
                    height={32}
                    className="rounded-full border border-outline-variant/30"
                  />
